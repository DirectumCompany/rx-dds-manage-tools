...
* в группу variables добавить:
  ...
  * переменную repositories, в которую будет записываться список репозиториев проекта
* в параметре common_config->CONNECTION_STRING использовать переменную database для задания базы данных
* рекомендуется переменные purpose, database, home_path, home_path_src и repositories переместить в начало блока variables
* по умолчанию каталог для логов сконфигурирован в домашний каталог проекта. Рекомендуется указать абсолютный путь, чтобы логи писались в одно место для разных проектов.
